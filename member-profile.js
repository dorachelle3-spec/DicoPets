...
(() => {
  ...
  let lastUserActivity=0,pointsBusy=false;
  const getPoints=user=>Math.max(0,Number(user?.user_metadata?.points||0));
  const activeNow=()=>document.visibilityState==='visible'&&document.hasFocus()&&Date.now()-lastUserActivity<75000;
  const markActivity=()=>{lastUserActivity=Date.now()};
  ['pointermove','keydown','click','scroll','touchstart'].forEach(event=>document.addEventListener(event,markActivity,{passive:true}));
  document.addEventListener('visibilitychange',()=>{if(document.visibilityState!=='visible')lastUserActivity=0});
  window.addEventListener('blur',()=>{lastUserActivity=0});
